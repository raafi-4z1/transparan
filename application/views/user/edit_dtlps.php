    ...
    <div class="col p-0">
      <div class="row m-2 shadow-sm mb-3 p-2 rounded" style="background-color: rgb(177, 236, 247)">
        ...
        <div class="col-sm-3 text-end">
          <a href="<?= base_url('user/dtlPesanan/'.$dpes['id_pemesanan']); ?>">
          <button type="button" class="btn btn-primary btn-sm rounded-pill float-sm-right shadow">Kembali</button>
          </a>
        </div>
      </div>    
      <div class="row m-2">
        <?= form_open_multipart('user/editDtlPesanan/'.$id); ?>
          <div class="form-row row">
            <div class="form-group col-sm-3">
              <label for="selectProduk">Produk</label>
              <input type="text" class="form-control" id="selectProduk" name="selectProduk" value="<?= $dpes['nama_produk']; ?>" disabled>
              <?= form_error('selectProduk', '<small class="text-danger pl-3">', '</small>'); ?>
            </div>
            <div class="form-group col-sm-3">
              <label for="selectBahan">Bahan</label>
              <input type="text" class="form-control" id="selectBahan" name="selectBahan" value="<?= $dpes['jenis_bahan']; ?>" disabled>
              <?= form_error('selectBahan', '<small class="text-danger pl-3">', '</small>'); ?>
            </div>
            <div class="form-group col-sm-3">
              <label for="selectSpesifikasi">Spesifikasi</label>
              <input type="text" class="form-control" id="selectSpesifikasi" name="selectSpesifikasi" value="<?= $dpes['spesifikasi']; ?>" disabled>
              <?= form_error('selectSpesifikasi', '<small class="text-danger pl-3">', '</small>'); ?>
            </div>
            <div class="form-group col-sm-3">
              <label for="inputUkuran">Ukuran</label>
              <input type="text" class="form-control" id="inputUkuran" name="inputUkuran" value="<?= $dpes['size']; ?>">
          ...
          <div class="form-row row">
            <div class="form-group col-sm-6">
              ...
              <div class="row">
                <div class="col-sm-4">
                  <img src="<?= $dpes['gambar'] == '' ? base_url('assets/img/default/no_image1.png') : 'data:image/jpeg;base64,'.base64_encode( $dpes['gambar'] ); ?>" class="img-thumbnail" style="max-width: 145px;">
                </div>
                <div class="col">
                  <input type="file" class="form-control-file m-2" id="inputGambar" name="inputGambar">
                  <?php if ($dpes['gambar'] != ''): ?>
                  <a href="<?= base_url('user/hapus/gdp/'.$id); ?>">
            ...
            <div class="form-group col-sm-2">
              ...
              <select class="form-control" id="selectLengan" name="selectLengan">
                <option value="Panjang" <?= $dpes['lengan_pe_pa'] == 'Panjang' ? 'selected' : '' ;?>>Panjang</option>
                <option value="Pendek" <?= $dpes['lengan_pe_pa'] == 'Pendek' ? 'selected' : '' ;?>>Pendek</option>
              </select>
              <?= form_error('selectLengan', '<small class="text-danger pl-3">', '</small>'); ?>
            </div>
            <div class="form-group col-sm-2">
              <label for="inputWarna">Warna</label>
              <input type="text" class="form-control" id="inputWarna" name="inputWarna" value="<?= $dpes['color']; ?>">
              <?= form_error('inputWarna', '<small class="text-danger pl-3">', '</small>'); ?>
            </div>
            <div class="form-group col-sm-2">
              <label for="inputQty">Jumlah Pesan <small id="dstok">Stok: <?= $dpes['stok']; ?></small></label>
              <input type="text" class="form-control" id="inputQty" name="inputQty" value="<?= $dpes['jumlah_pesanan']; ?>">
              <?= form_error('inputQty', '<small class="text-danger pl-3">', '</small>'); ?>
            </div>
          </div>
          <div class="form-group">
            <label for="formKeterangan">Keterangan</label>
            <textarea class="form-control" id="formKeterangan" name="formKeterangan" rows="3" placeholder="Tambahan: Sablon Area A3\Jumlah dibagi 2 warna..."><?= $dpes['deskripsi']; ?></textarea>
          </div>
